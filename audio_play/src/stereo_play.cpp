  ...
  class RosGstPlay
  {
    public:
      RosGstPlay()
      {
        ...
        ros::param::param<int>("~pan_value", pan_value, -1);
        ...
        if (dst_type == "alsasink")
        {
          ...
          _audiopanorama = gst_element_factory_make("audiopanorama", "audiopanorama");
          g_object_set(G_OBJECT(_audiopanorama), "panorama", gfloat(pan_value), NULL);
          ...
          gst_bin_add_many( GST_BIN(_audio), _convert, _audiopanorama, _resample, _sink, NULL);
          gst_element_link_many(_convert, _audiopanorama, _resample, _sink, NULL);
    ...
    private:
     ...
     static void cb_newpad (GstElement *decodebin, GstPad *pad, 
      ...
      GstElement *_pipeline, *_source, *_sink, *_decoder, *_convert, *_audiopanorama, *_audio, *_resample, *_filter;
      GstElement *_playbin;
      GMainLoop *_loop;
  };
}
